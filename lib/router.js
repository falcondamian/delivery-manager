...
Router.route('pedidos', {
    ...
    waitOn: function() { 
        ...
        return [Meteor.subscribe('pedidosOfReparto', this.params._id), Meteor.subscribe('clientesOfRuta', reparto.rutaId)];
    },
    data: function() { return repartos.findOne(this.params._id); }
});
...
Router.route('new_pedidos', {
    ...
    waitOn: function() { 
        /* Evitar cargar dos veces el reparto */
        var reparto = repartos.findOne(this.params._id);
        return Meteor.subscribe('clientesOfRuta', reparto.rutaId);  
    },      
    data: function() {        
        return {    
...
Router.route('show_pedidos', {
    ...
    waitOn: function() { 
        var reparto = repartos.findOne(this.params.repartoId);
        return [Meteor.subscribe('pedido', this.params._id), Meteor.subscribe('clientesOfRuta', reparto.rutaId)];  },
    data: function() { return pedidos.findOne(this.params._id); }
});
...
Router.route('pedidosRepartidor', {
    ...
    waitOn: function() {
        ...
        if (reparto) {
            ...
            return [Meteor.subscribe('pedidosOfReparto', reparto._id), 
                    Meteor.subscribe('clientesOfRuta', reparto.rutaId)];
...
Router.route('panelControl', {
    ...
    waitOn: function() {
        ...
        var idsRepartos = [];
        var repartosAux = repartos.find({fecha : today}).fetch();
        
        if (repartosAux && repartosAux.length > 0) {

            for(i = 0; i < repartosAux.length; i++) {

                idsRepartos.push(repartosAux[i]._id);
            }

            return [Meteor.subscribe('pedidosOfRepartos', idsRepartos), 
                    Meteor.subscribe('clientes'),
                    Meteor.subscribe('repartosFecha', today)];
